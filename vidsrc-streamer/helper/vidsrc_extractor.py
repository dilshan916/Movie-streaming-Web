...
class VidSrcExtractor:
    def __init__(self):
        self.scraper = c_requests.Session(impersonate="chrome120")
                ...
                if hls_url_match:
                    ...
                    if hls_url.endswith("/list.m3u8"):
                        ...
                        if set_pass_match and set_pass_match.group(1).startswith("//"):
                            ...
                            headers = HEADERS.copy()
                            headers["Referer"] = source
                            self.scraper.get(set_pass, headers=headers)
                        return hls_url
    ...
    def get_vidsrc_stream(self, name, media_type, code, language, season=None, episode=None) -> Optional[str]:
        ...
        url = f"https://vidsrc.net/embed/{media_type}?{provider}={code}"
        ...
        if not source:
            if not sources:
                ...
                return None, None
            first_source_name = list(sources.keys())[0]
            print(f"No source found for {name}, falling back to {first_source_name}")
            source = sources[first_source_name]

        headers = HEADERS.copy()
        headers["Referer"] = url
        req_1 = self.scraper.get(f"https://rcp.vidsrc.net/rcp/{source}", headers=headers)
        ...
        headers = HEADERS.copy()
        headers["Referer"] = f"https://rcp.vidsrc.net/rcp/{source}"
        req_2 = self.scraper.get(decoded_url, allow_redirects=False, headers=headers)
        ...
        if "vidsrc.stream" in location:
            return self.handle_vidsrc_stream(location, f"https://rcp.vidsrc.net/rcp/{source}"), subtitle
        if "2embed.cc" in location:
            print("[Warning] 2Embed does not work, this will not return anything!")
            return self.handle_2embed(location, f"https://rcp.vidsrc.net/rcp/{source}"), subtitle
        if "multiembed.mov" in location:
            return self.handle_multiembed(location, f"https://rcp.vidsrc.net/rcp/{source}"), subtitle
